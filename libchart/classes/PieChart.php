	...
	class PieChart extends Chart
	{
		...
		*/
		...
		function computePercent()
		{
			...
			foreach($this->point as $point)
			{
				$percent = 100 * $point->getY() / $this->total;
		...
		*/
		...
		{
			/*imagefilledrectangle($this->img, $x1, $y1, $x2, $y2, $this->axisColor1);
			imagerectangle($this->img, $x1, $y1, $x1 + 1, $y1 + 1, $this->axisColor2);
			imagerectangle($this->img, $x2 - 1, $y1, $x2, $y1 + 1, $this->axisColor2);
			imagerectangle($this->img, $x1, $y2 - 1, $x1 + 1, $y2, $this->axisColor2);
			imagerectangle($this->img, $x2 - 1, $y2 - 1, $x2, $y2, $this->axisColor2);*/
		}
		...
		*/
		...
		function createImage()
		{
			...
			foreach($pieColors as $colorRGB)
			{
				...
				$shadowColor = imagecolorallocate($this->img, $red * $shadowFactor, $green * $shadowFactor, $blue * $shadowFactor);
			...
			for($i = $this->pieTLY + 2; $i < $this->pieBRY - 1; $i++)
				imageline($this->img, $this->pieTLX + 2, $i, $this->pieBRX - 2, $i, $aquaColor[($i + 3) % 4]);*/
		}
		...
		*/
		...
		function printLabel()
		{
			...
			foreach($this->percent as $a)
			{
				...
				imagefilledrectangle($this->img, $boxX1 + 2, $boxY1 + 2, $boxX2 - 2, $boxY2 - 2, $color);
		...
		*/
		...
		function drawDisc($cy, $colorArray, $mode)
		{
			...
			foreach($this->percent as $a)
			{
				...
				imagefilledarc($this->img, $this->pieCenterX, $cy, $this->pieWidth, $this->pieHeight, $angle1, $angle2, $color, $mode);
		...
		*/
		...
		function printPie()
		{
			...
			for ($cy = $this->pieCenterY + $this->pieDepth / 2; $cy >= $this->pieCenterY - $this->pieDepth / 2; $cy--)
				$this->drawDisc($cy, $this->pieShadowColor, IMG_ARC_EDGED | IMG_ARC_NOFILL);


			// Top

			$this->drawDisc($this->pieCenterY - $this->pieDepth / 2, $this->pieColor, IMG_ARC_FILL);
